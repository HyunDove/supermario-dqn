...
- 전체 최고 보상은 EP 5000 시점(3,059)에서 사실상 수렴 — EP 10000 기준 3,062로 EP 5000 대비 0.1% 미만 개선에 그침

**DQN 한계 분석:**
- EP 5000 이후 전체 최고 보상이 3,059~3,062 수준에서 정체되며 **DQN 구조의 성능 상한(Performance Ceiling)**에 도달한 것으로 판단된다.
- 수치상 100회 평균 보상은 EP 7000(1,387) → EP 10000(1,590)으로 소폭 상승하였으나, 실제 플레이 영상에서는 눈에 띄는 진전 거리 향상이 나타나지 않았다.
- 이는 **DQN의 구조적 한계**에 기인한다: ① Policy Net이 Q값을 과대추정하는 Overestimation Bias 누적, ② ε이 0.1로 고정된 후 탐험 부재로 새로운 전략 발견이 어려움, ③ 동일 구간 경험이 Replay Buffer를 포화시켜 학습 다양성이 감소함.
- Double DQN(행동 선택과 Q값 평가를 분리), Prioritized Experience Replay(중요 경험 우선 학습) 등 개선 기법을 적용하면 이 정체 구간을 돌파할 수 있을 것으로 판단된다.
...
EP 0부터 EP 10000까지의 전체 보상 변화 및 100회 이동 평균을 나타낸 그래프다. 이동 평균이 EP 5000을 기점으로 상승 기울기가 완만해지며, EP 7000 이후에는 1,400~1,600 수준에서 횡보하는 양상을 보인다. 이는 DQN 에이전트가 현재 구조로 달성 가능한 성능 한계에 도달했음을 시각적으로 보여준다.
...
| EP 10000 | `gif/mario_ep10000.gif` | 최종 학습 결과 — 수치 향상에도 실질 전진 거리는 EP 7000과 유사 |
...
총 10,000 에피소드(1,593,659 스텝) 학습을 완료하였다. DQN 에이전트는 학습 초반부터 우측 전진 전략을 자율 습득하였으며, EP 5000을 기점으로 전체 최고 보상 3,062를 달성하는 등 유의미한 성능 향상이 관찰되었다.

그러나 **EP 5000 이후 전체 최고 보상은 3,059~3,062 수준에서 사실상 정체**되었으며, EP 10000 기준 100회 이동 평균 보상(1,590)은 EP 7000(1,387) 대비 소폭 상승에 그쳤다. 실제 플레이 영상에서도 EP 7000 이후 눈에 띄는 전진 거리 향상은 확인되지 않았다.

이는 현재 DQN 구조(Vanilla DQN)가 도달 가능한 **성능 상한(Performance Ceiling)**에 이른 것으로 판단되며, Double DQN·Prioritized Experience Replay 등 개선 기법의 적용이 필요한 단계임을 시사한다.
...
#### 1) 실험에서 관찰된 DQN 구조적 한계

본 과제에서 10,000 에피소드 학습을 완료한 결과, **EP 5000 이후 전체 최고 보상이 3,059~3,062 수준에서 정체**되며 Vanilla DQN의 성능 상한에 도달함을 확인하였다. EP 10000 기준 100회 평균 보상(1,590)은 EP 7000(1,387) 대비 수치상 소폭 향상되었으나, 실제 플레이 영상에서 전진 거리는 EP 7000과 동일하였다.

| 원인 | 내용 |
|---|---|
| **Q값 과대추정 (Overestimation Bias)** | Policy Net이 Q값을 체계적으로 과대추정하여, 학습이 진행될수록 잘못된 행동이 강화되는 경향 |
| **탐험 정체** | ε이 EP 2000 이전(약 230,000 스텝)에 이미 0.1로 수렴 — 이후 8,000 에피소드 동안 새로운 전략 발견 불가 |
| **Replay Buffer 포화** | 동일 구간(초중반 맵) 경험이 버퍼를 포화시켜 학습 다양성 감소 |
| **단일 스테이지 한계** | 1-1 스테이지만 학습하여 다양한 맵·상황에 대한 일반화 능력이 제한적 |
...
| **Prioritized Experience Replay** | TD 오차가 큰 중요 경험에 높은 샘플링 확률 부여 → 학습 효율 향상 |
| **Dueling DQN** | 상태 가치(V)와 행동 이점(A)을 분리 추정 → 행동 무관 상황에서 학습 안정화 |
| **ε 재탐험 스케줄** | 학습 정체 감지 시 ε을 일시적으로 높여 새 전략 탐험 유도 |

스테이지 클리어를 위해서는 위 개선 기법 적용과 함께 30,000 에피소드 이상의 추가 학습이 필요할 것으로 판단된다.
